import { pgTable, bigint, text, boolean, timestamp, varchar, uuid } from "drizzle-orm/pg-core"
...
export const application_log = pgTable("application_log", {
  // You can use { mode: "bigint" } if numbers are exceeding js number limitations
  id: bigint("id", { mode: "number" }).primaryKey().notNull(),
  created_date: timestamp("created_date", { withTimezone: true, mode: 'string' }).notNull(),
...
export const communication_block_list = pgTable("communication_block_list", {
  ...
  last_update_at: timestamp("last_update_at", { withTimezone: true, mode: 'string' }).notNull(),
});
...
export const whatsapp_template = pgTable("whatsapp_template", {
  ...
  template_id: text("template_id"),
  // You can use { mode: "bigint" } if numbers are exceeding js number limitations
  template_credential_id: bigint("template_credential_id", { mode: "number" }).notNull(),
  is_active: boolean("is_active").notNull(),
  created_at: timestamp("created_at", { withTimezone: true, mode: 'string' }).notNull(),
});

export const whatsapp_template_credentials = pgTable("whatsapp_template_credentials", {
  // You can use { mode: "bigint" } if numbers are exceeding js number limitations
  id: bigint("id", { mode: "number" }).primaryKey().notNull(),
  url: text("url"),
